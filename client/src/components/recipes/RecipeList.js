...
class RecipeList extends React.Component {
  ...
  renderList() {
    return this.props.recipes
      .filter((x) => x.userId === this.props.currentUserId)
      .map((recipe) => {
        return (
          <article key={recipe.id} className="card">
            ...
            <section>
              <Link to={`/recipes/${recipe.id}`} className="sectiune">
                <p>{recipe.title}</p>
      ...
      return (
        <article className="welcome--section">
          ...
          <lottie-player
            ...
            loop
            mode="normal"
            src="https://assets3.lottiefiles.com/packages/lf20_hssty91e.json"
            style={{ maxWidth: "460px", height:"320px" }}
          ></lottie-player>
        </article>
      );
    }
  ...
  renderSomething() {
    const recipes = this.props.recipes.filter(
      (x) => x.userId === this.props.currentUserId
    );

    if (recipes.length === 0 && this.props.isSignedIn) {
      return (
        <article className="welcome--section">
  ...
  renderCreate() {
    if (this.props.isSignedIn) {
      return (
        <Link to="/recipes/new" className="button--create">
          + Create Recipe
        </Link>
      );
    }
    ...
    return (
      <React.Fragment>
        ...
        {this.props.recipes.length > 0 && (
          <section className="wrapper-recipes">{this.renderList()}</section>
        )}
        {this.renderCreate()}
